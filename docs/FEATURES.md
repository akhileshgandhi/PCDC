...
| Case Attempt | `/student/case-studies/:id/attempt` | **Live** | The actual shipped flow has 4 stages, each backed by a real endpoint: Reading/Briefing → Initial Analysis (200-word gate) → Rapid Fire (3 AI-generated questions personalized to the student's own analysis) → Evaluation. A separate AI Discussion chat, Solution, Defense, and Reflection stage do not exist in the UI even though some backend/type scaffolding for them remains — an earlier version of this doc described those as shipped, which was aspirational rather than accurate. Initial Analysis drafts (text typed before submit) now autosave and survive a reload; other stages reconstruct from the DB on reload as before. |
